...
class Drawing_Number:
    ...
    def __init__(self, editstage):
        ...
        if self.editstage:
            self.caption = 'Edit drawing number'
            self.title = 'EDIT DRAWING NUMBER'
        else:
            self.caption = 'Add drawing number'
            self.title = 'ADD DRAWING NUMBER'
    ...
    def run(self):
        ...
        self.drawing_input = elements.InputBox_Userid(1, 3, 10, 1, views.product_data.product_data['drawing_number'], app = (self.screen), active = True, numpad_active = True)
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                self.drawing_value = self.drawing_input.handle_event(event)
