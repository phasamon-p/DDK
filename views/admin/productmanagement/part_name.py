...
class Part_Name:
    ...
    def __init__(self, editstage):
        ...
        if self.editstage:
            self.caption = 'Edit part name'
            self.title = 'EDIT PART NAME'
        else:
            self.caption = 'Add part name'
            self.title = 'ADD PART NAME'
    ...
    def next_click(self):
        if self.partname__value != '':
            if self.editstage:
                views.product_data.product_data['part_name'] = self.partname__value
    ...
    def cancel_click(self):
        if self.editstage:
            ...
            views.product_data.product_data['part_name'] = ''
    ...
    def run(self):
        ...
        self.partname_input = elements.InputBox_Text(1, 3, 10, 1, views.product_data.product_data['part_name'], app = (self.screen), active = True, numpad_active = True)
        ...
        while self.running:
            ...
            for row in range(12):
                ...
                for column in range(12):
                    ...
                    if row == 0 and column == 0:
                        elements.Title(self.title, pos=(360, 67), app=(self.screen)).draw()
                        ...
                        if self.editstage:
                            elements.Header_Table("  •  Please edit part name.", 1, 5, app=(self.screen)).draw()
                            elements.Header_Table("  •  If you don't want to edit, Please press next.", 1, 6, app=(self.screen)).draw()
                        else:
                            elements.Header_Table('  •  Please enter part name.', 1, 5, app=(self.screen)).draw()
                        if self.message:
                            elements.Output_Message("  •  Please enter part name.", 1, 10, app=(self.screen)).draw()
                        self.partname_input.draw()                  
                    """Initialize Button."""
            ...
            for event in pygame.event.get():
                self.partname__value = self.partname_input.handle_event(event)
